...
public class WorldStateForFullScene : MonoBehaviour, IListener, IProblemState
{
	...
	private void pickupBlock(String blockName)
	{
		foreach (GameObject block in gameObjects)
		{
			if (blockName.Equals(block.name, StringComparison.InvariantCultureIgnoreCase))
			{
				...
				moveToPos(target, true);
				jump(true);
				moveToPos(loadPosition.position + new Vector3(0, 0.03f, 0), true);
				moveToPos(loadPosition.position, true);
			}
		}
	}
